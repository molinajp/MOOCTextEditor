...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		for(int k=0;k<cArray.length;k++){
			if(k == cArray.length-1 && Character.toLowerCase(cArray[k]) == 'e' &&
					newSyllable && syllables > 0){
				syllables--;
			}
			if(newSyllable && vowels.indexOf(Character.toLowerCase(cArray[k])) >= 0){
				newSyllable = false;
				syllables++;
			}
			else if(vowels.indexOf(Character.toLowerCase(cArray[k])) < 0){
				newSyllable = true;
	...
	public double getFleschScore()
	{
		...
		double score = 206.835 - 1.015*(numWords/sentences)-84.6*(syllables/numWords);
	    return score;
	}
